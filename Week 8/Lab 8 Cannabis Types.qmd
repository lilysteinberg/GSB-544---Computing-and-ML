...
gscv = GridSearchCV(qda_pipeline, param_grid=tuning, cv = 5, scoring='f1_macro')
...
gscv = GridSearchCV(knn_pipeline, param_grid=neighbors, cv = 5, scoring='f1_macro')
...
```{python}
# get predictions
y_pred = cross_val_predict(knn_best_pipeline, X, y, cv=5)
...
**Q1: Build OvR for Logistic and SVC Models**
...
gscv = GridSearchCV(logistic_pipeline, param_grid=param_grid, cv = 5, scoring='f1_macro')
...
**Q2: Compare OvR for Logistic and SVC Models**
...
**Q3: Build OvO for Logistic and SVC Models**
...
**Q4: Compare OvO for Logistic and SVC Models**
...
Again, it makes intuitive sense that sativa and indica would be the easiest to differentiate, since hybrid probably shares some qualities with both. It makes sense that the logistic regression using the same class comparision performed second best. 

**Q5: Documentation application**
